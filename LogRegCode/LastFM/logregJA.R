...
wtrts = read.csv("SJCLFsub2traintest.csv")
wth = read.csv("SJCLFsub2hold.csv")
SJCLFsub2RTS = sparseMatrix(wtrts$row, wtrts$col, x=wtrts$val)
SJCLFsub2H = sparseMatrix(wth$row, wth$col, x=wth$val)
LFsub2S = LFsub2trts[,c("plays")]
YH = LFsub2hold[,c("plays")]

print("CV for log reg with L1 on SJCLFsub2RTS")
pt = proc.time(); trtsl1cv = cv.glmnet(SJCLFsub2RTS, LFsub2S, family="multinomial", type.measure="class", standardize=FALSE, nlambda=100, maxit=10000, thresh=0.001); print(proc.time() - pt)
print(trtsl1cv)
print(trtsl1cv$cvm)
...
print("Error of best CV L1 model on SJCLFsub2RTS")
 PLFsub2S = predict(trtsl1cv, SJCLFsub2RTS, s="lambda.min")
...
print("Error of best CV L1 model on SJCLFsub2H")
PYH = predict(trtsl1cv, SJCLFsub2H, s="lambda.min")
...
print("CV for log reg with L2 on SJCLFsub2RTS")
pt = proc.time(); trtsl2cv = cv.glmnet(SJCLFsub2RTS, LFsub2S, family="multinomial", type.measure="class", standardize=FALSE, alpha=0, nlambda=100, maxit=10000, thresh=0.001); print(proc.time() - pt)
print(trtsl2cv)
print(trtsl2cv$cvm)
...
print("Error of best CV L2 model on SJCLFsub2RTS")
PLFsub2S = predict(trtsl2cv, SJCLFsub2RTS, s="lambda.min")
PLFsub2Smax = rep('0', length(LFsub2S))
for (i in 1:length(LFsub2S)) {
	PLFsub2Smax[i] = names(which.max(PLFsub2S[i,,]))
}
...
print("Error of best CV L2 model on SJCLFsub2H")
PYH = predict(trtsl2cv, SJCLFsub2H, s="lambda.min")
PYHmax = rep('0', length(YH))
for (i in 1:length(YH)) {
	PYHmax[i] = names(which.max(PYH[i,,]))
}
