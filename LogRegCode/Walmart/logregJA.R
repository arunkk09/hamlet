...
wtrts = read.csv("SWTtraintest.csv")
wth = read.csv("SWThold.csv")
SWTRTS = sparseMatrix(wtrts$row, wtrts$col, x=wtrts$val)
SWTH = sparseMatrix(wth$row, wth$col, x=wth$val)
YRS = WTtrts[,c("weekly_sales")]
YH = WThold[,c("weekly_sales")]

print("CV for log reg with L1 on SWTRTS")
pt = proc.time(); trtsl1cv = cv.glmnet(SWTRTS, YRS, family="multinomial", type.measure="class", standardize=FALSE, nlambda=100, maxit=10000, thresh=0.001); print(proc.time() - pt)
print(trtsl1cv)
print(trtsl1cv$cvm)
...
print("Error of best CV L1 model on SWTRTS")
 PYRS = predict(trtsl1cv, SWTRTS, s="lambda.min")
...
print("Error of best CV L1 model on SWTH")
PYH = predict(trtsl1cv, SWTH, s="lambda.min")
...
print("CV for log reg with L2 on SWTRTS")
pt = proc.time(); trtsl2cv = cv.glmnet(SWTRTS, YRS, family="multinomial", type.measure="class", standardize=FALSE, alpha=0, nlambda=100, maxit=10000, thresh=0.001); print(proc.time() - pt)
print(trtsl2cv)
print(trtsl2cv$cvm)
...
print("Error of best CV L2 model on SWTRTS")
PYRS = predict(trtsl2cv, SWTRTS, s="lambda.min")
PYRSmax = rep('0', length(YRS))
for (i in 1:length(YRS)) {
	PYRSmax[i] = names(which.max(PYRS[i,,]))
}
...
print("Error of best CV L2 model on SWTH")
PYH = predict(trtsl2cv, SWTH, s="lambda.min")
PYHmax = rep('0', length(YH))
for (i in 1:length(YH)) {
	PYHmax[i] = names(which.max(PYH[i,,]))
}
